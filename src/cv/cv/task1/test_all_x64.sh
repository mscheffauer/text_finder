  ...
  then
    ...
    then
      convert -background 'rgba(0, 0, 0, .75)' -font DejaVu-Sans -fill white -pointsize 15 \
        label:" Different Sizes " -splice 0x3 $diff_img +swap -gravity north \
        -geometry +0+10 -composite $diff_img
    ...
    convert -background 'rgba(0, 0, 0, .75)' -font DejaVu-Sans -fill white -pointsize 15 \
      label:" Corrupt Image " -splice 0x3 $diff_img +swap -gravity north \
      -geometry +0+10 -composite $diff_img
